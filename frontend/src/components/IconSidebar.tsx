...
export const IconSidebar: React.FC<IconSidebarProps> = ({ selected, onSelect }) => {
  return (
    <aside className="w-12 bg-gray-200 dark:bg-gray-900 h-full flex flex-col items-center py-4 gap-4 border-r">
      <button
        className={`p-2 rounded ${selected === 'friends' ? 'bg-blue-500 text-white' : 'hover:bg-blue-100 dark:hover:bg-gray-700'}`}
      ...
      <button
        className={`p-2 rounded ${selected === 'chat' ? 'bg-blue-500 text-white' : 'hover:bg-blue-100 dark:hover:bg-gray-700'}`}
